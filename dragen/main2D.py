...
class DataTask2D:
    ...
    def __init__(self, box_size=38, n_pts=100, number_of_bands=0, bandwidth=3, shrink_factor=0.5, file1=None, file2=None, gui_flag=False):
    ...
    def initializations(self):
        ...
        phase1_a, phase1_b = self.utils_obj.read_input(phase1)
        final_volume_phase1 = [(phase1_a[i] * phase1_b[i] * np.pi) for i in range(len(phase1_a))]
        phase1_a_shrinked = [phase1_a[i] * self.shrink_factor for i in range(len(phase1_a))]
        phase1_b_shrinked = [phase1_b[i] * self.shrink_factor for i in range(len(phase1_b))]

        phase1_dict = {'a': phase1_a_shrinked, 'b': phase1_b_shrinked, 'final_volume': final_volume_phase1}
        grains_df = pd.DataFrame(phase1_dict)
        grains_df['phaseID'] = 1

        if phase2 is not None:
            phase2_a, phase2_b = self.utils_obj.read_input(phase2)
            ...
            phase2_dict = {'a': phase2_a_shrinked, 'b': phase2_b_shrinked, 'final_volume': final_volume_phase2}
    ...
    def rve_generation(self, epoch, grains_df):
        ...
        discrete_RSA_obj = DiscreteRsa2D(self.box_size, self.n_pts, grains_df['a'].tolist(),
                                         grains_df['b'].tolist(), store_path= store_path)
        ...
        if rsa_status:
            discrete_tesselation_obj = Tesselation2D(self.box_size, self.n_pts, grains_df['a'],
                                                     grains_df['b'], x_0_list, y_0_list,
                                                     self.shrink_factor, store_path)
            rve, rve_status = discrete_tesselation_obj.run_tesselation()
        else:
